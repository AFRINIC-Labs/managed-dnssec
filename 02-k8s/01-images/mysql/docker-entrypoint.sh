...
if [ "$SERVER_ID" ]; then
    echo "[Entrypoint] Creating custom my.cnf"
...
EOF
        ...
        if [ "$MYSQL_REPLICATION_MASTER_USER" -a "$MYSQL_REPLICATION_MASTER_USER_PASSWORD" ]; then
            echo "[Entrypoint] Creating replication user"
            echo "CREATE USER '"$MYSQL_REPLICATION_MASTER_USER"'@'%' IDENTIFIED BY '$MYSQL_REPLICATION_MASTER_USER_PASSWORD';" | "${mysql[@]}"
            echo "GRANT REPLICATION SLAVE ON *.* TO '"$MYSQL_REPLICATION_MASTER_USER"'@'%' ; " | "${mysql[@]}"
