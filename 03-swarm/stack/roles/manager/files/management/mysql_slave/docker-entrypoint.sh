...
echo "[Entrypoint-MySQL] MySQL Docker Image 5.7.27-1.1.12"
...
if [ "$SERVER_ID" ]; then
    echo "[Entrypoint-MySQL] Creating custom my.cnf"
...
if [ "$1" = 'mysqld' ]; then
	...
	if [ ! "$result" = "0" ]; then
		echo >&2 '[Entrypoint-MySQL] ERROR: Unable to start MySQL. Please check your configuration.'
		echo >&2 "[Entrypoint-MySQL] $output"
		exit 1
	fi
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		if [ -f "$MYSQL_ROOT_PASSWORD" ]; then
			...
			if [ -z "$MYSQL_ROOT_PASSWORD" ]; then
				echo >&2 '[Entrypoint-MySQL] Empty MYSQL_ROOT_PASSWORD file specified.'
				exit 1
			fi
		fi
		if [ -z "$MYSQL_ROOT_PASSWORD" -a -z "$MYSQL_ALLOW_EMPTY_PASSWORD" -a -z "$MYSQL_RANDOM_ROOT_PASSWORD" ]; then
			echo >&2 '[Entrypoint-MySQL] No password option specified for new database.'
			echo >&2 '[Entrypoint-MySQL]   A random onetime password will be generated.'
		...
		then
			for i in {30..0}; do
				...
				echo '[Entrypoint-MySQL] Waiting for server...'
				sleep 1
			done
			if [ "$i" = 0 ]; then
				echo >&2 '[Entrypoint-MySQL] Timeout during MySQL init.'
				exit 1
			fi
		fi
...
EOF
		...
		if [ "$MYSQL_USER" -a "$MYSQL_PASSWORD" ]; then
			...
			echo '[Entrypoint-MySQL] Not creating mysql user. MYSQL_USER and MYSQL_PASSWORD must be specified to create a mysql user.'
		fi
		echo
		for f in /docker-entrypoint-initdb.d/*; do
			case "$f" in
				*.sh)  echo "[Entrypoint-MySQL] running $f"; . "$f" ;;
				*.sql) echo "[Entrypoint-MySQL] running $f"; "${mysql[@]}" < "$f" && echo ;;
				*)     echo "[Entrypoint-MySQL] ignoring $f" ;;
			esac
			echo
		done

        # Create replication user on Master
        if [ "$REPLICATION_USER" -a "$REPLICATION_PASS" ]; then
            echo "[Entrypoint-MySQL] Creating replication user"
            echo "CREATE USER '"$REPLICATION_USER"'@'%' IDENTIFIED BY '$REPLICATION_PASS';" | "${mysql[@]}"
		...
		#rm -f "$PASSFILE"
		unset PASSFILE
		echo "[Entrypoint-MySQL] Server shut down"

		# This needs to be done outside the normal init, since mysqladmin shutdown will not work after
		if [ ! -z "$MYSQL_ONETIME_PASSWORD" ]; then
			if [ -z "yes" ]; then
				echo "[Entrypoint-MySQL] User expiration is only supported in MySQL 5.6+"
			else
				echo "[Entrypoint-MySQL] Setting root user as expired. Password will need to be changed before database can be used."
...
EOF
		...
		echo
		echo '[Entrypoint-MySQL] MySQL init process done. Ready for start up.'
		echo
	fi
...
EOF
	...
	echo "[Entrypoint-MySQL] Starting MySQL 5.7.27-1.1.12"
fi
